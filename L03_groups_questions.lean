...
#check g ^ (-1) -- doesnt know how to deal with Neg ℕ (as takes 1 : ℕ)
...
example : (a⁻¹)⁻¹ = a := by
  exact inv_inv a --exact? --ask in a group setting about this
  done

lemma aux2 : a * (b * c) * a⁻¹ = (a * b * a⁻¹) * (a * c * a⁻¹) := by
  exact conj_mul.symm --exact?
  done

lemma aux : a * (b * c) * a⁻¹ = (a * b * a⁻¹) * (a * c * a⁻¹) := by
  simp only [conj_mul] --simp?
  done

example : a⁻¹ * (b * c) * a = (a⁻¹ * b * a) * (a⁻¹ * c * a) := by
  -- exact? gives nothing
  --simp? gives nothing
  simp [← mul_assoc] --explain what ← does here! replaces RHS with LHS instead of LHS with RHS
  done
...
example : addOrderOf (1 : ℤ) = 0 := by
  rw [addOrderOf_eq_zero_iff'] --' at end!
  intros n n0 --puts the assumptions in the logic statements of the goal into our assumption area
  simp
  exact Nat.pos_iff_ne_zero.mp n0 --exact?
  done
